...
- **Conso estimée corrigée des coupures de pompe** (v3.45.2) : `apport = débit × Δt` suppose la **pompe en marche continue**, or elle est **intermittente** → sur tout
  intervalle où le **bassin ne monte pas**, `débit×Δt` **surestime** l'apport (donc la conso). Constaté en prod : série 50–138 m³/j et tendance ~81,5 m³/j alors que le réel
  ≈ 18,8 m³/j. **Source unique** `utils/consoEstimee.ts` (`calculerConsoEstimee`, pur) : la conso n'est **directement observable** que sur les intervalles de **VIDAGE**
  (le niveau baisse, pompe à l'arrêt → conso = `−Δstock`, sans hypothèse de pompe) ; les intervalles **MONTANTS/PLATS** (conso masquée par le remplissage) sont **estimés**
...
> 🛰️ **Rôle Promoteur dans l'invitation (Phase 3 promoteur, v3.45.0)** : le formulaire « Nouvelle invitation » propose **Promoteur** en plus d'Administrateur/Releveur/Client (sur les **2 canaux** email & WhatsApp ; `eau_invitations.role_promoteur`), la validation d'une **demande reçue** permet aussi d'attribuer Promoteur, et les invitations portent un **badge Promoteur**. À l'enrôlement (login de l'invité), **les deux RPC** `eau_claim_invitation()` et `eau_claim_invitation_by_token()` **octroient `eau_roles.promoteur`** (insert + on-conflict). Un promoteur (lecture seule, Phase 2) ne peut ni créer d'invitation ni valider de demande. Cible d'atterrissage promoteur = `/gestion-eau` (tableau de bord lecture).
