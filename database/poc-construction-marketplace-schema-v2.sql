-- ============================================================================
-- SCHEMA POC CONSTRUCTION MARKETPLACE - BazarKELY (VERSION 2)
-- ============================================================================
-- SCHEMA VERSION 2 - SUPABASE COMPATIBLE
-- Fix: CREATE TYPE now uses DO blocks (IF NOT EXISTS not supported)
-- Date: 2025-01-21
-- Agent 1: Database Architecture Design
-- Version: 2.0.0 (Supabase compatible - DO blocks for ENUMs)
--
-- SCHEMA FIXES FOR SUPABASE COMPATIBILITY
-- Date: 2025-01-21
-- Issue: PostgreSQL CHECK constraints cannot contain subqueries (EXISTS, IN with SELECT, etc.)
-- Error: "ERROR: 0A000: cannot use subquery in check constraint LINE 187"
--
-- Changes Made:
--   1. Removed CHECK constraint 'check_supplier_type' on poc_products (line 186-191)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = supplier_id AND type = 'supplier'))
--      - Replaced with: Trigger validation function validate_product_supplier_type()
--
--   2. Removed CHECK constraint 'check_builder_type' on poc_projects (line 218-223)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = company_id AND type = 'builder'))
--      - Replaced with: Trigger validation function validate_project_builder_type()
--
--   3. Removed CHECK constraint 'check_buyer_is_builder' on poc_purchase_orders (line 286-291)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = buyer_company_id AND type = 'builder'))
--      - Replaced with: Trigger validation function validate_purchase_order_buyer_type()
--
--   4. Removed CHECK constraint 'check_supplier_is_supplier' on poc_purchase_orders (line 292-297)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = supplier_company_id AND type = 'supplier'))
--      - Replaced with: Trigger validation function validate_purchase_order_supplier_type()
--
--   5. Removed CHECK constraint 'check_inventory_builder_type' on poc_inventory_items (line 375-380)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = company_id AND type = 'builder'))
--      - Replaced with: Trigger validation function validate_inventory_builder_type()
--
--   6. Removed CHECK constraint 'check_stock_movement_builder_type' on poc_stock_movements (line 402-407)
--      - Was: CHECK (EXISTS (SELECT 1 FROM poc_companies WHERE id = company_id AND type = 'builder'))
--      - Replaced with: Trigger validation function validate_stock_movement_builder_type()
--
-- All functionality preserved through trigger-based validation.
-- FOREIGN KEY constraints remain unchanged and provide referential integrity.
-- All other CHECK constraints (without subqueries) remain unchanged.
-- ============================================================================
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_company_type') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_company_status') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_member_role') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_member_status') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_project_status') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_order_status') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_stock_movement_type') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'poc_stock_reference_type') THEN
...
CREATE TABLE IF NOT EXISTS public.poc_products (
  ...
  -- NOTE: CHECK constraint 'check_supplier_type' removed (contained subquery)
  -- Validation now handled by trigger validate_product_supplier_type()
);
...
CREATE TABLE IF NOT EXISTS public.poc_projects (
  ...
  -- NOTE: CHECK constraint 'check_builder_type' removed (contained subquery)
  -- Validation now handled by trigger validate_project_builder_type()
...
CREATE TABLE IF NOT EXISTS public.poc_purchase_orders (
  ...
  -- NOTE: CHECK constraints 'check_buyer_is_builder' and 'check_supplier_is_supplier' removed (contained subqueries)
  -- Validation now handled by triggers validate_purchase_order_buyer_type() and validate_purchase_order_supplier_type()
...
CREATE TABLE IF NOT EXISTS public.poc_inventory_items (
  ...
  -- NOTE: CHECK constraint 'check_inventory_builder_type' removed (contained subquery)
  -- Validation now handled by trigger validate_inventory_builder_type()
);
...
CREATE TABLE IF NOT EXISTS public.poc_stock_movements (
  ...
  -- NOTE: CHECK constraint 'check_stock_movement_builder_type' removed (contained subquery)
  -- Validation now handled by trigger validate_stock_movement_builder_type()
);
...
-- ============================================================================
-- SECTION 4: VALIDATION TRIGGERS (Replacement for removed CHECK constraints)
-- ============================================================================

-- Fonction de validation: vérifier que supplier_id est de type 'supplier'
CREATE OR REPLACE FUNCTION public.validate_product_supplier_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
    WHERE id = NEW.supplier_id AND type = 'supplier'
  ) THEN
    RAISE EXCEPTION 'supplier_id must reference a company of type ''supplier''';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Fonction de validation: vérifier que company_id est de type 'builder' (pour projets)
CREATE OR REPLACE FUNCTION public.validate_project_builder_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
    WHERE id = NEW.company_id AND type = 'builder'
  ) THEN
    RAISE EXCEPTION 'company_id must reference a company of type ''builder''';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Fonction de validation: vérifier que buyer_company_id est de type 'builder'
CREATE OR REPLACE FUNCTION public.validate_purchase_order_buyer_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
    WHERE id = NEW.buyer_company_id AND type = 'builder'
  ) THEN
    RAISE EXCEPTION 'buyer_company_id must reference a company of type ''builder''';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

-- Fonction de validation: vérifier que supplier_company_id est de type 'supplier'
CREATE OR REPLACE FUNCTION public.validate_purchase_order_supplier_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
    WHERE id = NEW.supplier_company_id AND type = 'supplier'
  ) THEN
...
-- Fonction de validation: vérifier que company_id est de type 'builder' (pour inventaire)
CREATE OR REPLACE FUNCTION public.validate_inventory_builder_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
...
-- Fonction de validation: vérifier que company_id est de type 'builder' (pour mouvements de stock)
CREATE OR REPLACE FUNCTION public.validate_stock_movement_builder_type()
RETURNS TRIGGER AS $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM public.poc_companies
...
CREATE TRIGGER validate_product_supplier_type_trigger
  BEFORE INSERT OR UPDATE OF supplier_id ON public.poc_products
  FOR EACH ROW EXECUTE FUNCTION public.validate_product_supplier_type();

CREATE TRIGGER validate_project_builder_type_trigger
  BEFORE INSERT OR UPDATE OF company_id ON public.poc_projects
  FOR EACH ROW EXECUTE FUNCTION public.validate_project_builder_type();

CREATE TRIGGER validate_purchase_order_buyer_type_trigger
  BEFORE INSERT OR UPDATE OF buyer_company_id ON public.poc_purchase_orders
  FOR EACH ROW EXECUTE FUNCTION public.validate_purchase_order_buyer_type();

CREATE TRIGGER validate_purchase_order_supplier_type_trigger
  BEFORE INSERT OR UPDATE OF supplier_company_id ON public.poc_purchase_orders
  FOR EACH ROW EXECUTE FUNCTION public.validate_purchase_order_supplier_type();

CREATE TRIGGER validate_inventory_builder_type_trigger
  BEFORE INSERT OR UPDATE OF company_id ON public.poc_inventory_items
  FOR EACH ROW EXECUTE FUNCTION public.validate_inventory_builder_type();

CREATE TRIGGER validate_stock_movement_builder_type_trigger
  BEFORE INSERT OR UPDATE OF company_id ON public.poc_stock_movements
  FOR EACH ROW EXECUTE FUNCTION public.validate_stock_movement_builder_type();
...
-- ============================================================================
-- SECTION 6: TRIGGERS (Automatisation)
-- ============================================================================
...
COMMENT ON SCHEMA public IS 'Schema POC Construction Marketplace - Tables isolées avec préfixe poc_ (Version corrigée pour Supabase)';
