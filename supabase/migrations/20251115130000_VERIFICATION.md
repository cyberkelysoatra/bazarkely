# Vérification Migration: Add company_id to poc_org_unit_members

## Migration File
`20251115130000_add_company_id_to_org_unit_members.sql`

## Objectif
Ajouter la colonne `company_id` à la table `poc_org_unit_members` pour permettre le filtrage direct par compagnie dans les requêtes (utilisé par PurchaseOrderForm.tsx ligne 252).
...
### 2. Vérifier que la contrainte de clé étrangère existe
...
```sql
-- Vérifier que tous les membres ont un company_id
SELECT 
...
### 5. Vérifier que company_id correspond à org_unit.company_id

```sql
-- Vérifier la cohérence entre company_id et org_unit.company_id
SELECT 
  COUNT(*) as total_members,
...
### Test 1: Requête depuis PurchaseOrderForm (devrait réussir)

```sql
-- Simuler la requête de PurchaseOrderForm.tsx ligne 252
...
- `company_id` est présent dans les résultats

---

### Test 2: Vérifier les performances avec index

```sql
-- Test d'explain pour vérifier que l'index est utilisé
EXPLAIN ANALYZE
SELECT org_unit_id 
FROM public.poc_org_unit_members 
...
**Résultat attendu:**
- Plan d'exécution utilise l'index `idx_poc_org_unit_members_company_active`
- Temps d'exécution rapide

---

### Test 3: Vérifier la contrainte de clé étrangère

```sql
-- Tenter d'insérer un company_id invalide (devrait échouer)
...
**Résultat attendu:**
- ❌ INSERT échoue avec erreur de contrainte de clé étrangère
- Message: `violates foreign key constraint "fk_org_unit_members_company"`

---

### Test 4: Vérifier la cohérence après insertion

```sql
-- Insérer un nouveau membre et vérifier que company_id est cohérent
INSERT INTO public.poc_org_unit_members (
  org_unit_id,
  user_id,
  role,
...
-- Vérifier que company_id correspond à org_unit.company_id
SELECT 
  m.id,
...
**Note:** Si le company_id n'est pas automatiquement rempli lors de l'insertion, il faudra créer un trigger pour le faire automatiquement.
...
```sql
-- Vérifier que les policies RLS existantes fonctionnent toujours
SELECT 
  policyname,
  cmd,
  CASE 
...
- Optionnel: Les policies peuvent être optimisées pour utiliser `company_id` directement
...
UNION ALL
SELECT 
  'Incohérences company_id (ERREUR)',
  COUNT(*)::text
FROM public.poc_org_unit_members m
INNER JOIN public.poc_org_units u ON m.org_unit_id = u.id
...
**Résultat attendu:**
- Les deux dernières lignes doivent retourner `0` (aucune erreur)
...
**Prérequis pour rollback:**
- Vérifier qu'aucun code ne dépend de la colonne `company_id`
- S'assurer que les requêtes peuvent fonctionner sans cette colonne

---

## 🚀 Optimisation Future (Optionnel)

Pour maintenir automatiquement `company_id` lors des insertions/updates, créer un trigger:
...
RETURNS TRIGGER AS $$
BEGIN
  -- Si company_id n'est pas fourni, le dériver de org_unit_id
...
Après avoir exécuté tous les tests ci-dessus, vous devriez avoir:

- [x] Colonne `company_id` existe dans `poc_org_unit_members`
- [x] Contrainte de clé étrangère `fk_org_unit_members_company` existe
- [x] 4 index créés pour les performances
- [x] Tous les enregistrements existants ont un `company_id` valide
- [x] `company_id` correspond à `org_unit.company_id` pour tous les enregistrements
- [x] Requête depuis PurchaseOrderForm.tsx fonctionne sans erreur
