...
import { ChevronRight, Inbox, RefreshCw, Store, Truck, WifiOff } from 'lucide-react';
...
export default function OperatorRequestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await listPartners({ statuses: ['pending'] });
      setRows(data);
      setNavyProfile({ pendingCount: data.length });
    } catch (err) {
  ...
  return (
    <NavyPage>
      ...
      {isOnline && rows && !loading && rows.length > 0 && (
        <button type="button" className={btnSecondary} onClick={() => void load()}>
          <RefreshCw className="w-4 h-4" aria-hidden="true" />
          Actualiser
        </button>
      ...
      <NavyHelp title="Comment traiter une demande ?">
        ...
        <p>Validez si tout est bon. Sinon, refusez en choisissant un motif : la personne le verra, pourra corriger et renvoyer.</p>
