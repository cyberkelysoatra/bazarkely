...
function niveauClasses(niveau: string | null): string {
  switch (niveau) {
    ...
    default:
      return 'border-ahuvi-100 bg-white';
  }
}
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <div className="text-xs font-semibold uppercase tracking-wide text-gray-500">
                          ...
                          {!a.lu && <span className="ml-2 text-[10px] text-ahuvi-gold-700">● nouveau</span>}
                        </div>
                      ...
                      {!isReadOnly && (
                        <div className="flex flex-col gap-1 flex-shrink-0">
                          <button
                            onClick={() => traiter(a)}
                            className="inline-flex items-center justify-center gap-1 min-h-[44px] text-xs font-semibold text-emerald-700 bg-emerald-100 hover:bg-emerald-200 px-3 py-1.5 rounded-lg transition-colors"
                          >
                            <Check className="w-3.5 h-3.5" /> Traité
                          </button>
                          <button
                            onClick={() => basculerLu(a)}
                            className="inline-flex items-center justify-center gap-1 min-h-[44px] text-xs text-gray-500 hover:text-gray-700 hover:bg-gray-100 px-3 py-1.5 rounded-lg transition-colors"
                          >
                            <Eye className="w-3.5 h-3.5" /> {a.lu ? 'Non lu' : 'Lu'}
                          </button>
                        </div>
                      )}
                    </div>
