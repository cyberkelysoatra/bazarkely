/**
 * Saisie compteur ÉLECTRIQUE (sous-onglet « Électricité » des Relevés — releveur/admin).
 * Copie adaptée de EauSaisieCompteurPage en kWh : recherche → liste groupée par zone →
 * sélection → dernier index kWh + date → saisie nouvel index → conso instantanée.
 * Gère rupture (index <) + aberrant. MÊMES compteurs que l'eau (eau_compteurs).
 */
import { useEffect, useMemo, useState } from 'react';
import toast from 'react-hot-toast';
import { ResponsiveContainer, BarChart, Bar, XAxis, Tooltip } from 'recharts';
import {
  ChevronLeft, ChevronRight, Camera, Save, Zap, AlertTriangle, Flag, Trash2, BarChart3,
} from 'lucide-react';
...
import type { CompteurLocal, ElecReleveLocal } from '../types/gestionEau';
...
export default function EauSaisieElecPage({
  preselectCompteurId,
}: {
  /** Compteur à préselectionner (deep-link depuis la tournée / un scan). */
  ...
  // Préselection directe (choix depuis la tournée / un scan) : dès que la liste est
  // chargée, sélectionne le compteur ciblé sans action manuelle.
  useEffect(() => {
  ...
  const selectCompteur = async (c: CompteurLocal) => {
    ...
    setHisto([]);
    try {
      const hist = await historiqueConsoElec(c.id);
  ...
  useEffect(() => {
    ...
    let alive = true;
    (async () => {
      const ev = await evaluerReleveElec(selected.id, n);
  ...
  const submit = async () => {
    ...
    const ev = evalState ?? (await evaluerReleveElec(selected.id, n));
    ...
    if (!ev.ruptureIndex && ev.aberrant.aberrant) {
      ...
      const ok = await showConfirm(
        `La consommation calculée (${ev.conso} kWh) est ${sens} par rapport à l'historique. Confirmer quand même ?`,
        'Relevé aberrant',
        { variant: 'warning', confirmText: 'Confirmer' }
      );
      if (!ok) return;
    ...
    try {
      ...
      toast.success(
        ev.ruptureIndex
          ...
          : `Relevé enregistré — conso ${fmtKwh(ev.conso)}`
      );
      setSelected(null);
      setIndexStr('');
      setNote('');
  ...
  return (
    <EauPageShell title="Relevé électricité" subtitle="Relevé d'index des compteurs électriques (kWh)" aide={AIDE.elecReleves}>
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-soft space-y-3">
          ...
          <div className="flex items-center gap-2">
            <EauListIcon icon={Zap} tone="gold" />
            <div>
          ...
          {histo.length > 0 && (
            <div className="rounded-lg border border-ahuvi-100 bg-ahuvi-50/40 p-3">
              <div className="flex items-center gap-1.5 text-xs text-gray-500 mb-1">
                <BarChart3 className="w-3.5 h-3.5" aria-hidden="true" /> Consommation par période (kWh)
              </div>
          ...
          <label className="text-sm block">
            <span className="block text-gray-600 mb-1">Nouvel index kWh</span>
            <input
              type="number"
              inputMode="decimal"
              step="0.001"
          ...
          {evalState && (
            <div className="space-y-1">
              {evalState.ruptureIndex ? (
                ...
                <div className="text-sm text-ahuvi-olive bg-amber-50/60 rounded-lg px-3 py-2">
                  Consommation : <strong>{fmtKwh(evalState.conso)}</strong>
                </div>
              )}
        ...
        <div className="space-y-3">
          ...
          />
          {grouped.length === 0 ? (
            <EauEmptyState icon={Zap} title="Aucun compteur actif" hint="Aucun compteur ne correspond à votre recherche." />
          ) : (
                  ...
                  {list.map((c) => (
                    ...
                    >
                      <span className="flex items-center gap-2 min-w-0">
                        <EauListIcon icon={Zap} tone="neutral" />
...
/** Affiche le dernier index élec + date du compteur sélectionné. */
function DernierIndex({ compteurId, ev }: { compteurId: string; ev: Evaluation | null }) {
  const [dernier, setDernier] = useState<ElecReleveLocal | null | undefined>(ev?.dernier ?? undefined);
  ...
  useEffect(() => {
    ...
    let alive = true;
    (async () => {
      const d = await getDernierReleveElec(compteurId);
  ...
  return (
    <span className="text-gray-700">
      Dernier index : <strong>{dernier.index}</strong> kWh le {fmtDate(dernier.timestamp)}
    </span>
  );
}
