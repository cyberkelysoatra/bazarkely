...
export const VERSION_HISTORY = [
  {
    version: '3.45.1',
    date: '2026-06-09',
    description: 'fix(gestion-eau) : conso estimée réaliste — apport plafonné à la pompe intermittente. Validé chiffré en prod : la conso estimée (et la projection anti-zéro qui en hérite) surestimait massivement (tendance ~81,5 m³/j vs réel ~18,8 m³/j ; série 50–138 m³/j). Cause : computeBilan utilise apport = débit×Δt en supposant la pompe en marche continue, or elle se coupe au flotteur → débit×Δt surestime l\'apport sur tout intervalle parqué au niveau plein. Nouveau helper PUR utils/consoEstimee.ts (calculerConsoEstimee + consoBaseM3hOf, 9 tests) : classe chaque intervalle FIABLE (fini sous τ=0,95×V_flotteur → garde apport−Δstock) vs PLAFONNÉ (fini au flotteur, apport débit → remplace par consoBase×Δt). consoBase = moyenne consoReseau/h des intervalles fiables (anti-circularité) ; replis estimerAutonomie.consoMoyenneHeureM3 → débit×FRACTION_POMPE(0,5) → 0. Net de pertes = ×(1−0,30). Une entrée manuelle ne plafonne jamais. eauTendanceService + eauBilanService rebranchés sur ce helper (SOURCE UNIQUE) ; bucket par jour LOCAL (bucketByLocalDay) cohérent avec le tableau de bord et la projection. computeBilan/bilans persistés/NRW/Conso réseau période INCHANGÉS. tsc OK, build OK, 107 tests eau verts.',
    changes: [
      'NOUVEAU modules/gestion-eau/utils/consoEstimee.ts : calculerConsoEstimee + consoBaseM3hOf (pur)',
      ...
      'constants/appVersion.ts + package.json : version 3.45.1 + note FR',
    ],
    type: 'patch' as const,
  },
  {
