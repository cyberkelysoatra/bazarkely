...
export const VERSION_HISTORY = [
  ...
  },
  {
    ...
    changes: [
      ...
      'components/Layout/AppLayout.tsx [PARTAGÉ] : overscroll-y-auto sur /gestion-eau (élastique), overscroll-none ailleurs (Cœur/Construction inchangés)',
  ...
  },
  {
    ...
    date: '2026-06-15',
    description:
      "style(eau) : tableau de bord — carte « Conso au compteur » réduite à UNE ligne de hint (`cumulSub(flux.consoM3)`). Retrait de la 2ᵉ ligne `consoJourHint` (origine du chiffre, « estimée/mesurée »), de la fonction `consoJourHint` devenue inutilisée et de l'import de type `ConsoJourSource` (noUnusedLocals). `TrendingUp` reste utilisé (liens Tendances). Présentationnel. tsc OK, build OK.",
    changes: [
  ...
  },
  {
    ...
    date: '2026-06-15',
    description:
      "feat(eau) : Conso du réseau / NRW — Phase 2 (moteur « débit × temps de marche »). `utils/bilan.ts` : la sortie réseau est DÉCOUPLÉE du bilan de matière. Nouveau : `heuresArretSurIntervalle` (Σ recouvrements des arrêts de pompe avec ]tPrev,t]) + input `arrets` + `ArretPompeLite`. `consoReseauM3 = apportReseau − Δstock` où apportReseau = override > entrées > `débit × (Δt − arrêts)` (PLUS de plafond flotteur, PLUS de FRACTION_POMPE) ; `null` si débit inconnu ou sortie ≤ 0. `pertesM3`/`nrwReseauPct` → nullable (= eau non comptée). `apportM3`/stockAttendu/écart/anomalie de STOCK inchangés (bilan de matière conservé). `eauBilanService` : charge `eau_arrets_pompe` + passe `arrets` ; l'anomalie stockée ne folde PLUS `anomalieReseau` (eau non comptée = normale). Dashboard inchangé d'aspect (carte NRW renommée en Phase 3). Recalcul requis : « Recalculer tous les bilans ». Tests : 70 OK (eauBassinDebit mis à jour au nouveau modèle + 3 nouveaux : arrêts déduits, arrêt total → null, sans débit → null). tsc OK, build OK.",
    changes: [
      'utils/bilan.ts : + heuresArretSurIntervalle/ArretPompeLite + input arrets ; consoReseau = débit × temps de marche − Δstock (nullable) ; pertes/nrw nullable ; apport mass-balance + écart/anomalie inchangés',
