...
export default function HeaderEauActions() {
  ...
  const { roles, realRoles } = useGestionEau();
    ...
    <div className="relative ml-auto" ref={ref}>
      ...
      {open && (
        <div className="absolute top-full right-0 mt-2 bg-white rounded-xl border border-ahuvi-100 shadow-2xl z-50 min-w-[240px] overflow-hidden">
          ...
          {/* 🎭 Simulation de rôle — ADMIN RÉEL uniquement. Une SEULE ligne sobre (sans
              état) qui ouvre la page dédiée (/gestion-eau/simulation). Reste TOUJOURS
              visible (gate sur realRoles.admin, PAS sur les rôles effectifs) pour que
              l'admin puisse rouvrir la page et sortir/changer de simulation quel que soit
              le rôle incarné. Tous les réglages (rôle + villa) vivent désormais dans la page. */}
          {realRoles?.admin && (
            <div className="border-b border-ahuvi-100 bg-ahuvi-gold/5">
              <button
                onClick={() => go('/gestion-eau/simulation')}
                className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-ahuvi-800 hover:bg-ahuvi-gold/10 transition-colors font-ahuvi-body"
              >
                <Drama className="w-4 h-4 text-ahuvi-gold-700" />
                <span className="flex-1 text-left font-medium">Simulation de rôle</span>
